...
**GitHub Actions:** workflow [`.github/workflows/ios_screenshots.yml`](.github/workflows/ios_screenshots.yml) builds `lib/main_screenshots.dart` on the iOS simulator and captures PNGs with `simctl io`. It pins **Xcode 16** (not latest) and **Flutter 3.44.0** because `google_mlkit_*` excludes arm64 for the simulator and **Xcode 26** arm-only simulators fail with `Framework 'Pods_Runner' not found` ([flutter#182748](https://github.com/flutter/flutter/issues/182748)). Requires repo secrets `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `GOOGLE_WEB_CLIENT_ID`.
