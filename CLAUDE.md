...
**On `debrief`:** follow the vault `CLAUDE.md` debrief flow — include **vault ship check** ([DEFINITION_OF_DONE.md](DEFINITION_OF_DONE.md) step 2c) when `pubspec.yaml` or GitHub is ahead of vault release notes. End with **copy-paste commit message(s) only** (vault `docs: …` and/or `halal_checker` when code changed); **no `git` commands** in the reply; do not commit unless asked.
